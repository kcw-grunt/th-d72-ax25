...
var kissTNC = function(args) {
	...
	this.__defineGetter__("serialPort",function() {return properties.serialPort;});
	...
	this.__defineGetter__("baudRate",function() {return properties.baudRate;});

	this.__defineSetter__(
		"txDelay",
		function(txDelay) {
			if(	typeof txDelay != "number"||txDelay < 0||txDelay > 255) {
	...
	this.__defineGetter__("txDelay",function() {return properties.txDelay * 10;});

	this.__defineSetter__(
		"persistence",
		function(persistence) {
			if(	typeof persistence != "number"||persistence < 0||persistence > 1) {
	...
	this.__defineGetter__("persistence",function() {return (properties.persistence / 256) + 1;});

	this.__defineSetter__(
		"slotTime",
		function(slotTime) {
			if(	typeof slotTime != "number"||slotTime < 0||slotTime > 255) {
	...
	this.__defineGetter__("slotTime",function() {return properties.slotTime * 10;});
	
	this.__defineSetter__(
		"txTail",
		function(txTail) {
			if(	typeof txTail != "number"||txTail < 0||txTail > 255) {
	...
	this.__defineGetter__("txTail",function() {return properties.txTail * 10;});
	...
	this.__defineSetter__(
		...
		function(fullDuplex) {
			...
			sendFrame(ax25.kissDefs.FULLDUPLEX,[(properties.fullDuplex) ? 1 : 0]);
		}
	);
	
	this.__defineGetter__("fullDuplex",function() {return (properties.fullDuplex == 1) ? true : false;});
	...
	const parser = serialHandle.pipe(new Readline({ delimiter: '\r\n' }))
	...
	this.enterD72KISS = function() {
		...
		serialHandle.write('CONV\r\n'); 
		console.log('Entered TH-D72A into KISS Mode Conv');
	}
